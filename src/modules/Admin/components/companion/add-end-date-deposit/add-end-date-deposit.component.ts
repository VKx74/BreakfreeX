...
@Component({
    selector: 'add-end-date-deposit',
    templateUrl: './add-end-date-deposit.component.html',
    styleUrls: ['./add-end-date-deposit.component.scss']
})
export class AddEndDateDepositComponent {
    ...
    add() {
        if (!this.publicKey || !this.token || !this.amount || !this.days) {
        ...
        if (!this.publicKey || !this.token || this.amount <= 0 || this.days <= 0) {
            this._alertService.error("Incorrect data");
            return;
        }

        this.loading = true;
        this._companionUserTrackerService.addEndDateDeposit({
            amount: this.amount,
