...
(function () {
    ...
    function _getString(key, lang) {
        ...
        if (lang === 'en') {
            ...
            strings = {
                bold: "強大 <strong> Ctrl+B",
                boldexample: "強文本",

                italic: "重點 <em> Ctrl+I",
                italicexample: "強調文字強調文字",

                link: "超鏈接 <a> Ctrl+L",
                linkdescription: "在此輸入鏈接描述",
                linkdialog: "插入超鏈接",

                quote: "大段引用 <blockquote> Ctrl+Q",
                quoteexample: "大段引用",

                code: "代碼示例 <pre><code> Ctrl+K",
                codeexample: "在此處輸入代碼",

                image: "圖片 <img> Ctrl+G",
                imagedescription: "在此輸入圖像描述",

                olist: "編號表 <ol> Ctrl+O",
                ulist: "項目符號列表 <ul> Ctrl+U",
                litem: "列表項",

                heading: "標題 <h1>/<h2> Ctrl+H",
                headingexample: "標題",
    ...
    Markdown.Editor = function (markdownConverter, idPostfix, options) {
        ...
        var getString = function (identifier) {
            return _getString(identifier, this.lang);
        }.bind(this);
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString) {
        ...
        function makeSpritedButtonRow() {
            ...
            var xPosition = 0;
            var makeButton = function (selector, textOp) {
            ...
            buttons.bold = makeButton('.control-bold', bindCommand("doBold"));

            buttons.italic = makeButton('.control-italic', bindCommand("doItalic"));

            buttons.link = makeButton('.control-link', bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, false);
            }));
            buttons.quote = makeButton('.control-quote', bindCommand("doBlockquote"));
            buttons.code = makeButton('.control-code', bindCommand("doCode"));
            buttons.image = makeButton('.control-img', bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, true);
            }));


            buttons.olist = makeButton('.control-olist', bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.ulist = makeButton('.control-ulist', bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.heading = makeButton('.control-header', bindCommand("doHeading"));
            buttons.hr = makeButton('.control-hr', bindCommand("doHorizontalRule"));
            buttons.undo = makeButton('.control-undo', null);
            ...
            buttons.redo = makeButton('.control-redo', null);
    ...
    commandProto.doLinkOrImage = function (chunk, postProcessing, isImage) {
        ...
        if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {
            ...
            if (isImage) {
                ui.onShowAddImageDialog(linkEnteredCallback)
            } else {
                ui.onShowAddLinkDialog(linkEnteredCallback);
            }
