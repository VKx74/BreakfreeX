...
lm.utils.DragListener = function( eElement, nButtonCode, allowDragging ) {
	...
	this._allowDragging = allowDragging;
...
lm.utils.copy( lm.utils.DragListener.prototype, {
	...
	onMouseDown: function( oEvent ) {
		...
		if( (oEvent.button == 0 || oEvent.type === "touchstart")  && (!this._allowDragging || this._allowDragging())) {
...
lm.items.AbstractContentItem = function( layoutManager, config, parent ) {
    ...
    Object.defineProperty(this, "isMaximised", {
        get: function () {
            return layoutManager._maximisedItem === this;
        }
    });
...
lm.utils.copy( lm.items.AbstractContentItem.prototype, {
	...
	replaceChild: function( oldChild, newChild, _$destroyOldChild ) {
		...
		if (oldChild.isMaximised) {
			parentNode.replaceChild( newChild.element[ 0 ], this.layoutManager._maximisePlaceholder[0] );
		} else {
...
lm.utils.copy( lm.items.RowOrColumn.prototype, {
	...
	addChild: function( contentItem, index, _$suspendResize ) {
		...
		if( this.contentItems.length > 0 ) {
			...
			if( index > 0 ) {
				if (this.contentItems[ index - 1 ].isMaximised) {
					this.layoutManager._maximisePlaceholder.after( splitterElement );
				} else {
					this.contentItems[ index - 1 ].element.after( splitterElement );
				}

				if (contentItem.isMaximised) {
					splitterElement.after( this.layoutManager._maximisePlaceholder );
				} else {
					splitterElement.after( contentItem.element );
				}
			} else {
				if (this.contentItems[ 0 ].isMaximised) {
					this.layoutManager._maximisePlaceholder.before( splitterElement );
				} else {
					this.contentItems[ 0 ].element.before( splitterElement );
				}

				if (contentItem.isMaximised) {
					splitterElement.before( this.layoutManager._maximisePlaceholder );
				} else {
			...
			if (contentItem.isMaximised) {
				this.childElementContainer.append( this.layoutManager._maximisePlaceholder );
			} else {
				this.childElementContainer.append( contentItem.element );
			}
		}
	...
	_setAbsoluteSizes: function() {
		...
		for( i = 0; i < this.contentItems.length; i++ ) {
			...
			var layoutContainer = this.layoutManager.container;

			if (this.contentItems[i].isMaximised) {
				this.contentItems[ i ].element.width( layoutContainer.width() );
				this.contentItems[ i ].element.height( layoutContainer.height() );
	...
	_onSplitterDragStop: function( splitter ) {
		...
		lm.utils.animFrame( handler.bind(this) );
	}
} );
...
lm.utils.copy( lm.items.Stack.prototype, {
	...
	_$init: function() {
		...
		if( this.contentItems.length > 0 ) {
			...
			this.setActiveContentItem( initialItem, false );
		}
	},

	setActiveContentItem: function( contentItem, emitStateChange ) {
	...
	removeChild: function( contentItem, keepChild ) {
		...
		if (this.header.activeContentItem === contentItem) {
			...
			if (this._activeContentItem) {
				this.config.activeItemIndex = lm.utils.indexOf(this._activeContentItem, this.contentItems);
			}
		}
...
lm.controls.Tab = function( header, contentItem ) {
	...
	if(
		...
		this._dragListener = new lm.utils.DragListener( this.element, undefined,  lm.utils.fnBind( this._allowDragging, this ));
		this._dragListener.on( 'dragStart', this._onDragStart, this );
...
lm.utils.copy( lm.controls.Tab.prototype, {
	...
	_allowDragging: function () {
		return this._layoutManager.root.getItemsByType('component').length > 1;
	}
} );
